  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      {nodes.map((node, i) => (
        ...
        >
          <div className="node-glow w-3 h-3 rounded-full bg-emerald-400 border border-emerald-300/50" />
          <span className="text-[10px] text-emerald-400/70 font-mono tracking-wider">{node.label}</span>
        </motion.div>
      ))}

      {/* Center logo + dual orbits */}
      <motion.div
      ...
      >
        ...
        <div className="px-4 py-3 space-y-1">
          {codeLines.map((line, i) => (
            ...
            >
              <span className="text-gray-700 mr-3 select-none w-3 text-right shrink-0">{i + 1}</span>
              <span
                style={{ paddingLeft: `${line.indent * 12}px` }}
                className={
                  line.text.startsWith("//") ? "text-gray-600"
                  : line.text.includes("const") || line.text.includes("await") ? "text-teal-300"
                  : line.text.includes("true") || line.text.includes("Infinity") ? "text-emerald-400"
                  : "text-gray-400"
                }
              >
                {line.text}
    ...
    <main className="relative min-h-screen bg-[#050807] text-white flex overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="orb-pulse absolute top-[-200px] left-[-150px] w-[600px] h-[600px]
                        bg-gradient-to-br from-emerald-500/10 to-transparent rounded-full blur-[180px]" />
        <div className="orb-pulse absolute bottom-[-200px] right-[-100px] w-[500px] h-[500px]
                        bg-gradient-to-tl from-teal-500/8 to-transparent rounded-full blur-[160px]" />
      </div>
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center px-8 py-12 relative">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="mb-8">
            ...
            <p className="text-sm text-gray-600 mt-1.5">Sign in to access your client dashboard.</p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-gray-700">or continue with email</span>
            <div className="flex-1 h-px bg-white/[0.06]" />
          </motion.div>
          ...
          <motion.form variants={itemVariants} onSubmit={handleLogin} className="space-y-4">
            ...
            {info && (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl
                              bg-emerald-500/8 border border-emerald-500/20 text-emerald-400 text-xs">
                <CheckCircle2 size={13} className="shrink-0" /> {info}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="cta-btn w-full py-3 rounded-xl text-sm font-semibold text-black
          ...
          <motion.div variants={itemVariants} className="mt-5 flex items-center justify-between">
            ...
            <p className="text-xs text-gray-600">
              No account?{" "}
              <Link to="/signup" className="text-white hover:text-emerald-300 transition-colors font-medium">
                Sign up
              </Link>
            </p>
          </motion.div>
